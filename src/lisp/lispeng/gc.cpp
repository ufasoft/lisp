...
void CValueManBase::FillEmpty(size_t i) {
...
size_t CValueManBase::ScanType(CObMap& obMap) {
	...
	size_t count = 0;
	byte *p = (byte*)m_pBase;
	for (size_t i=0; i<m_size; ++i, p+=m_sizeof) {
#if UCFG_LISP_GC_USE_ONLY_BITMAP
...
void CValueManBase::Resize(size_t n) {
	...
	AlignedFree(m_pBase);
	delete[] (BASEWORD*)exchange(m_pBitmap, new BASEWORD[n/BASEWORD_BITS]);
...
//!!!R	*(end+1) = exchange(m_pHeap, (byte*)m_pBase+i*m_sizeof);


	//!!!D *(end+1) = exchange(m_pHeap, end);
}
	...
	for (int i=0; i<m_size; ++i, p+=m_sizeof) {
		if (!BitOps::BitTest(m_pBitmap, i) && *p!=0xFF) {
			...
			if (!bSimple)
				Delete(p);
			*p = 0xFF;
			*(void**)(p+sizeof(CP)) = exchange(pHeap, p);
		}
	}
	m_pHeap = pHeap;
#endif
...
void CLispEng::CollectEx() {
	...
	for (size_t i=0; i<m_weakPointerMan.m_size; ++i) {
