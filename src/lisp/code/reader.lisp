       ...
       (let* ((token (read-token stream)))
         (unless *read-suppress*
           (unless (or [Escape-Zeichen im Token verwendet]
             ...
             (error "~ von ~: Nach #* durfen nur Nullen und Einsen kommen."
                     'read stream))
           (let ((l (length token)))
             (if n
               (cond ((< n l)
                      (error "~ von ~: Bit-Vektor langer als angegebene Lange ~."
                              'read stream n))
                     ((and (plusp n) (zerop l))
                      (error "~ von ~: Element fur Bit-Vektor der Lange ~ muss spezifiziert werden."
                              'read stream n)))
               (setq n l))
